    ...
    if (etat.enmouvement)
    {
        ...
        if (etat.impression)
        {
            ...
            if (nresul)
            {
                ...
                if (nresul > NRETOUR)
                    nresul = NRETOUR; // on veut seulement les NRETOUR premières particules
...
void chargerNuanceurs()
{
    ...
    std::cout << "Charger le nuanceur de ce TP" << std::endl;
    {
        ...
        if ((locColor = glGetAttribLocation(prog, "Color")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de Color" << std::endl;
        if ((loctempsRestant = glGetAttribLocation(prog, "tempsRestant")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de tempsRestant" << std::endl;
        if ((locvitesse = glGetAttribLocation(prog, "vitesse")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de vitesse" << std::endl;
        ...
        if ((locmatrProj = glGetUniformLocation(prog, "matrProj")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de matrProj" << std::endl;
        if ((loclaTexture = glGetUniformLocation(prog, "laTexture")) == -1)
    ...
    std::cout << "Charger le nuanceur de rétroaction" << std::endl;
    {
        ...
        // demander la "Location" des variables
        if ((locpositionRetroaction = glGetAttribLocation(progRetroaction, "position")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de position" << std::endl;
        if ((locvitesseRetroaction = glGetAttribLocation(progRetroaction, "vitesse")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de vitesse" << std::endl;
        if ((loccouleurRetroaction = glGetAttribLocation(progRetroaction, "couleur")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de couleur" << std::endl;
        if ((loctempsRestantRetroaction = glGetAttribLocation(progRetroaction, "tempsRestant")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de tempsRestant" << std::endl;
        if ((loctempsRetroaction = glGetUniformLocation(progRetroaction, "temps")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de temps" << std::endl;
        if ((locdtRetroaction = glGetUniformLocation(progRetroaction, "dt")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de dt" << std::endl;
        if ((locgraviteRetroaction = glGetUniformLocation(progRetroaction, "gravite")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de gravite" << std::endl;
        if ((loctempsMaxRetroaction = glGetUniformLocation(progRetroaction, "tempsMax")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de tempsMax" << std::endl;
        if ((locpositionPuitsRetroaction = glGetUniformLocation(progRetroaction, "positionPuits")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de positionPuits" << std::endl;
        if ((locbDimRetroaction = glGetUniformLocation(progRetroaction, "bDim")) == -1)
            std::cerr << "!!! pas trouvé la \"Location\" de bDim" << std::endl;
    }
}
...
void FenetreTP::clavier(TP_touche touche)
{
    ...
    case TP_CROCHETDROIT:
    {
        ...
        if (parametres.nparticules > MAXNPARTICULES)
            parametres.nparticules = MAXNPARTICULES;
    ...
    case TP_CROCHETGAUCHE:
        parametres.nparticules /= 1.2;
        if (parametres.nparticules < 5)
            parametres.nparticules = 5;
        std::cout << " nparticules=" << parametres.nparticules << std::endl;
        break;
    ...
    case TP_MOINS: // Reculer la caméra
        camera.dist += 0.2;
        if (camera.dist > 20.0 - etat.bDim.y)
            camera.dist = 20.0 - etat.bDim.y;
        std::cout << " camera.dist=" << camera.dist << std::endl;
        break;
    ...
    case TP_h: // Décrémenter la gravité
        parametres.gravite -= 0.05;
        if (parametres.gravite < 0.0)
            parametres.gravite = 0.0;
        std::cout << " parametres.gravite=" << parametres.gravite << std::endl;
        break;
    ...
    case TP_k: // Décrémenter la durée de vie maximale
        parametres.tempsMax -= 0.2;
        if (parametres.tempsMax < 1.0)
            parametres.tempsMax = 1.0;
        std::cout << " parametres.tempsMax=" << parametres.tempsMax << std::endl;
        break;
...
static enum { deplaceCam,
              deplacePuits } deplace = deplaceCam;
...
void FenetreTP::sourisWheel(int x, int y) // Changer la distance de la caméra
{
    ...
    if (camera.dist < 0.4)
        camera.dist = 0.4;
    else if (camera.dist > 20.0 - etat.bDim.y)
        camera.dist = 20.0 - etat.bDim.y;
}
...
void FenetreTP::sourisMouvement(int x, int y)
{
    if (pressed)
    {
        ...
        case deplacePuits:
        {
            ...
            etat.positionPuits = glm::unProject(ecranPos, VM, P, cloture);
            etat.verifierPositionPuits();
        }
        break;
        }
